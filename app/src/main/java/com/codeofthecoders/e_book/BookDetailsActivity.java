...
public class BookDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Top Reading Book");

        mList = findViewById(R.id.main_list_book);
            ...
            public void onResponse(JSONArray response) {
                for (int i = 0; i < response.length(); i++) {
                    try {
                        ...
                        ReadBook category = new ReadBook();
                        category.setImg_url(jsonObject.getString("book_img"));
                        category.setName(jsonObject.getString("book_name"));
                        category.setRate(jsonObject.getString("book_rate"));


                        readBookList.add(category);
